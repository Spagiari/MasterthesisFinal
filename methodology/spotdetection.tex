    ...
    \node [block, below of=im_ori] (at_blk) {Transf BOTTOM-HAT};
    \node [block, right of=at_blk] (ts_ini) {BT = 0};
    \node [block, below of=ts_ini] (im_bin) {Binarizar imagem ponto de corte igual a BT};
    \node [decision, below of=im_bin, node distance=5.5cm] (bl_chk) {Novos pixels pretos $>$ MA ?};
    ...
    \node [decision, below of=mm_opn, node distance=4.5cm] (wh_chk) {Aumentou número de pixels brancos?};
    % Draw edges
    ...
    \path [line] (bl_chk) |- node{não} (wh_chk);
    \path [line] (bl_chk) -| node{sim} (ts_inc);
    \path [line] (ts_inc) -- (im_bin);
    \path [line] (wh_chk) -- node{não} (mm_opn);
    \path [line] (wh_chk) -| node{sim} (se_inc);
